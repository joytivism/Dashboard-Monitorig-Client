    ...
    <aside className="w-[248px] h-screen sticky top-0 bg-surface border-r border-border-main flex flex-col shrink-0 z-40 uppercase" style={{ boxShadow: '2px 0 16px -4px rgba(0,0,0,0.06)' }}>
      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-1 no-scrollbar">
        <div className="px-3 py-2 mb-1">
           <span className="text-[9px] font-black text-text4 tracking-[0.12em]">Main Navigation</span>
        </div>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 text-[11px] font-black transition-all duration-200 group ${
                isActive 
      ...
      <div className="p-4 border-t border-border-main bg-surface2/50">
        <div className="bg-white rounded-2xl p-4 border border-border-main shadow-sm mb-3">
           <div className="flex items-center gap-3 mb-4">
              ...
              <div className="flex flex-col min-w-0">
                 <span className="text-[10px] font-black text-text truncate leading-none">Admin User</span>
                 <span className="text-[9px] font-bold text-text3 tracking-wider mt-1.5">Superuser</span>
              </div>
           </div>
           
           <Link 
             href="/" 
             className="flex items-center justify-between w-full h-9 px-3 rounded-xl text-[9px] font-black text-text2 hover:bg-surface2 hover:text-text transition-all border border-border-main/50 group"
           >
        ...
        <form action={logout}>
          <button 
            type="submit"
            className="flex items-center gap-3 w-full px-3.5 py-2.5 rounded-xl text-[10px] font-black text-rr tracking-wider hover:bg-rr-bg transition-all group"
          >
