    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="bg-white rounded-3xl p-8 border border-border-main shadow-sm space-y-6">
        ...
        <div>
          ...
          <div className="text-3xl font-bold text-text tracking-tight">{stats.tofu.reach > 0 ? fK(stats.tofu.reach) : '—'}</div>
        </div>
        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border-main/50">
          <div><span className="text-xs font-bold text-text3 block mb-0.5">Impresi</span><span className="font-bold text-text">{stats.tofu.impr > 0 ? fK(stats.tofu.impr) : '0'}</span></div>
          <div><span className="text-xs font-bold text-text3 block mb-0.5">Cost/Reach</span><span className="font-bold text-text">{stats.tofu.reach > 0 && stats.tofu.sp > 0 ? fRp(stats.tofu.sp / stats.tofu.reach) : '—'}</span></div>
        </div>
        <div className="flex justify-between items-center bg-surface2 rounded-2xl p-4">
          ...
          <span className="font-bold text-tofu">{stats.tofu.sp > 0 ? fRp(stats.tofu.sp) : 'Rp 0'}</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-8 border border-border-main shadow-sm space-y-6">
        ...
        <div>
          ...
          <div className="text-3xl font-bold text-text tracking-tight">{stats.mofu.vis > 0 ? fK(stats.mofu.vis) : '—'}</div>
        </div>
        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border-main/50">
          <div><span className="text-xs font-bold text-text3 block mb-0.5">Orders</span><span className="font-bold text-text">{stats.mofu.ord > 0 ? fK(stats.mofu.ord) : '0'}</span></div>
          <div><span className="text-xs font-bold text-text3 block mb-0.5">Shop Rev.</span><span className="font-bold text-gg">{stats.mofu.rev > 0 ? fK(stats.mofu.rev) : '0'}</span></div>
        </div>
        <div className="flex justify-between items-center bg-surface2 rounded-2xl p-4">
          <span className="text-sm font-bold text-text2">Conversion</span>
          <span className="font-bold text-mofu">{stats.mofu.vis > 0 ? ((stats.mofu.ord / stats.mofu.vis) * 100).toFixed(2) : 0}%</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-8 border border-border-main shadow-sm space-y-6">
        ...
        <div>
          ...
          <div className="text-3xl font-bold text-text tracking-tight">{stats.bofu.ord > 0 ? fK(stats.bofu.ord) : '—'}</div>
        </div>
        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border-main/50">
          <div><span className="text-xs font-bold text-text3 block mb-0.5">ROAS</span><span className="font-bold text-text">{roas ? roas.toFixed(2) + 'x' : '—'}</span></div>
          <div><span className="text-xs font-bold text-text3 block mb-0.5">CHK Rate</span><span className="font-bold text-text">{stats.mofu.vis > 0 ? ((stats.bofu.ord / stats.mofu.vis) * 100).toFixed(2) : 0}%</span></div>
        </div>
        <div className="flex justify-between items-center bg-surface2 rounded-2xl p-4">
          <span className="text-sm font-bold text-text2">Ad Revenue</span>
          <span className="font-bold text-gg">{stats.bofu.rev > 0 ? fRp(stats.bofu.rev) : 'Rp 0'}</span>
        </div>
      </div>
    </div>
  );
