  ...
  return (
    <div className="space-y-6 max-w-[1400px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
        {/* TOFU */}
        <div className="bg-white rounded-[24px] p-8 shadow-main relative overflow-hidden group">
          ...
          <div className="relative z-10 flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-base font-bold text-text">TOFU</h3>
                <p className="text-xs font-bold text-blue-500 uppercase tracking-wider mt-0.5">Awareness</p>
              </div>
            </div>
          </div>
          ...
          <div className="relative z-10 space-y-6">
            {tofu_reach > 0 && (
              <div>
                <div className="text-xs font-semibold text-text3 mb-1 uppercase tracking-wider">Total Reach</div>
                <div className="text-3xl font-bold text-text tracking-tight">{fK(tofu_reach)}</div>
              </div>
            )}
            
            <div className="grid grid-cols-2 gap-4 pt-5 mt-2">
              {tofu_impr > 0 && <div><span className="text-[11px] font-semibold text-text3 block mb-1 uppercase">Impresi</span><span className="font-bold text-sm text-text">{fK(tofu_impr)}</span></div>}
              {tofu_res > 0 && <div><span className="text-[11px] font-semibold text-text3 block mb-1 uppercase">Results</span><span className="font-bold text-sm text-text">{fK(tofu_res)}</span></div>}
            </div>
            
            {tofu_sp > 0 ? (
               <div className="flex justify-between items-center bg-surface2 rounded-xl p-4 mt-4"><span className="text-xs font-semibold text-text2">Ad Spend</span><span className="font-bold text-accent">{fRp(tofu_sp)}</span></div>
            ) : <div className="text-xs text-text3 italic pt-2">No awareness active</div>}
          </div>
        </div>

        {/* MOFU */}
        <div className="bg-white rounded-[24px] p-8 shadow-main relative overflow-hidden group">
          <div className="absolute -right-6 -top-6 w-32 h-32 bg-yellow-50 rounded-full blur-3xl group-hover:bg-yellow-100 transition-colors"></div>
          ...
          <div className="relative z-10 flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-base font-bold text-text">MOFU</h3>
                <p className="text-xs font-bold text-yellow-500 uppercase tracking-wider mt-0.5">Consideration</p>
              </div>
            </div>
          </div>
          ...
          <div className="relative z-10 space-y-6">
            <div>
              <div className="text-xs font-semibold text-text3 mb-1 uppercase tracking-wider">Total Orders</div>
              <div className="text-3xl font-bold text-text tracking-tight">{mofu_ord.toLocaleString('id-ID')}</div>
            </div>
            
            <div className="grid grid-cols-2 gap-4 pt-5 mt-2">
              {mofu_vis > 0 && <div><span className="text-[11px] font-semibold text-text3 block mb-1 uppercase">Visitors</span><span className="font-bold text-sm text-text">{fK(mofu_vis)}</span></div>}
              {mofu_vis > 0 && mofu_ord > 0 && <div><span className="text-[11px] font-semibold text-text3 block mb-1 uppercase">Conv. Rate</span><span className="font-bold text-sm text-text">{(mofu_ord/mofu_vis*100).toFixed(2)}%</span></div>}
            </div>
            
            <div className="flex justify-between items-center bg-surface2 rounded-xl p-4 mt-4"><span className="text-xs font-semibold text-text2">Shop Revenue</span><span className="font-bold text-green-600">{fRp(mofu_rev)}</span></div>
          </div>
        </div>

        {/* BOFU */}
        <div className="bg-white rounded-[24px] p-8 shadow-main relative overflow-hidden group">
          <div className="absolute -right-6 -top-6 w-32 h-32 bg-green-50 rounded-full blur-3xl group-hover:bg-green-100 transition-colors"></div>
          ...
          <div className="relative z-10 flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-base font-bold text-text">BOFU</h3>
                <p className="text-xs font-bold text-green-500 uppercase tracking-wider mt-0.5">Conversion</p>
              </div>
            </div>
          </div>
          ...
          <div className="relative z-10 space-y-6">
            <div>
              <div className="text-xs font-semibold text-text3 mb-1 uppercase tracking-wider">Ad Revenue</div>
              <div className="text-3xl font-bold text-text tracking-tight">{fRp(bofu_rev)}</div>
            </div>
            
            <div className="grid grid-cols-2 gap-4 pt-5 mt-2">
              <div><span className="text-[11px] font-semibold text-text3 block mb-1 uppercase">Ad Spend</span><span className="font-bold text-sm text-text2">{fRp(bofu_sp)}</span></div>
              {blCir && <div><span className="text-[11px] font-semibold text-text3 block mb-1 uppercase">CIR</span><span className="font-bold text-sm text-text">{blCir.toFixed(1)}%</span></div>}
            </div>
            
            <div className="flex justify-between items-center bg-surface2 rounded-xl p-4 mt-4"><span className="text-xs font-semibold text-text2">ROAS</span><span className="font-bold text-text">{bRoas ? bRoas.toFixed(2) + 'x' : '—'}</span></div>
          </div>
        </div>
      </div>
